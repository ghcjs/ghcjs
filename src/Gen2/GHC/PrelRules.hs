...
-- workaround for incorrect truncation with 64 bit host, 32 bit target
-- (truncating to Word or Int gives incorrect results if the host/target
-- word size are different)
mkWordLitWord32 :: DynFlags -> Word32 -> Expr b
mkWordLitWord32 dflags w = Lit (mkMachWord dflags (toInteger w))

mkIntLitInt32 :: DynFlags -> Int32 -> Expr b
mkIntLitInt32 dflags n = Lit (mkMachInt dflags (toInteger n))
...
builtinIntegerRules =
 [rule_IntToInteger   "smallInteger"        smallIntegerName,
  ...
  rule_convert        "integerToWord"       integerToWordName       mkWordLitWord32,
  rule_convert        "integerToInt"        integerToIntName        mkIntLitInt32,
...
match_Integer_convert convert dflags id_unf _ [xl]
  ...
  = -- Debug.Trace.trace ("integer convert match: " ++ show x)
