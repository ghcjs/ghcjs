...
import qualified Compiler.InteractiveEval as IE
...
flushInterpBuffers
 = liftIO $ hFlush stdout >> hFlush stderr
   {- do getHandle stdout_ptr >>= hFlush
               getHandle stderr_ptr >>= hFlush -}

turnOffBuffering :: IO ()
turnOffBuffering
 = do -- hdls <- mapM getHandle [stdin_ptr,stdout_ptr,stderr_ptr]
      mapM_ (\h -> hSetBuffering h NoBuffering) [stdin, stdout, stderr]
{-
